...
class Refiner(nn.Module):
    ...
    __constants__ = ['kernel_size',
                     'patch_crop_method', 'patch_replace_method']
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(
            channels[0] + 6 + 4 + 1, channels[1], kernel_size, bias=False)
        self.bn1 = nn.BatchNorm2d(channels[1])
        self.conv2 = nn.Conv2d(
            channels[1], channels[2], kernel_size, bias=False)
        self.bn2 = nn.BatchNorm2d(channels[2])
        self.conv3 = nn.Conv2d(
            channels[2] + 6, channels[3], kernel_size, bias=False)
        self.bn3 = nn.BatchNorm2d(channels[3])
        self.conv4 = nn.Conv2d(
            channels[3], channels[4], kernel_size, bias=True)
    ...
    def forward(self,
        ...
        if self.mode != 'full':
            ...
            if idx[0].size(0) > 0:
                x = torch.cat([hid, pha, fgr, depth], dim=1)
                x = F.interpolate(x, (H_half, W_half),
                                  mode='bilinear', align_corners=False)
                x = self.crop_patch(
                    x, idx, 2, 3 if self.kernel_size == 3 else 0)

                y = F.interpolate(src_bgr, (H_half, W_half),
                                  mode='bilinear', align_corners=False)
                y = self.crop_patch(
                    y, idx, 2, 3 if self.kernel_size == 3 else 0)
                ...
                x = F.interpolate(x, 8 if self.kernel_size ==
                                  3 else 4, mode='nearest')
                y = self.crop_patch(
                    src_bgr, idx, 4, 2 if self.kernel_size == 3 else 0)
    ...
    def select_refinement_regions(self, err: torch.Tensor):
        ...
        if self.mode == 'sampling':
            ...
            b, _, h, w = err.shape
            err = err.view(b, -1)
            idx = err.topk(self.sample_pixels // 16,
    ...
    def crop_patch(self,
        ...
        if self.patch_crop_method == 'unfold':
            ...
            pat = pat.view(-1, x.size(1), size + 2 *
                           padding, size + 2 * padding)
            return pat
    ...
    def replace_patch(self,
        ...
        if self.patch_replace_method == 'scatter_nd':
            ...
            x = x.view(xB, xC, xH // yH, yH, xW // yW,
                       yW).permute(0, 2, 4, 1, 3, 5)
    ...
    def compute_pixel_indices(self,
        ...
        idx_pat = (c * H * W).view(C, 1, 1).expand([C, O, O]) + (o * W).view(
            1, O, 1).expand([C, O, O]) + o.view(1, 1, O).expand([C, O, O])
