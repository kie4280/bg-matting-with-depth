...
class Base(nn.Module):
    ...
    def __init__(self, backbone: str, in_channels: int, out_channels: int):
        ...
        if backbone in ['resnet50', 'resnet101']:
            ...
            self.decoder = Decoder([256, 128, 64, 48, out_channels], [
                                   512, 256, 64, in_channels])
        else:
            self.backbone = MobileNetV2Encoder(in_channels)
            self.aspp = ASPP(320, [3, 6, 9])
            self.decoder = Decoder([256, 128, 64, 48, out_channels], [
                                   32, 24, 16, in_channels])
...
class MattingBase(Base):
    ...
    def __init__(self, backbone: str):
        super().__init__(backbone, in_channels=6, out_channels=(1 + 3 + 1 + 32 + 1))
