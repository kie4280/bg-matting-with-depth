...
Example:
    ...
    CUDA_VISIBLE_DEVICES=0 python3 inference_video.py \
...
parser.add_argument('--video-target-bgr', type=str, default=None,
                    help="Path to video onto which to composite the output (default to flat green)")
...
parser.add_argument('--output-types', type=str, required=True, nargs='+',
                    choices=['com', 'pha', 'fgr', 'err', 'ref', 'depth', 'depthdiff'])
parser.add_argument('--output-format', type=str,
                    default='video', choices=['video', 'image_sequences'])
...
class VideoWriter:
    def __init__(self, path, frame_rate, width, height):
        self.out = cv2.VideoWriter(path, cv2.VideoWriter_fourcc(
...
class ImageSequenceWriter:
    ...
    def _add_batch(self, frames, index):
        ...
        for i in range(frames.shape[0]):
            ...
            frame.save(os.path.join(self.path, str(
                index + i).zfill(5) + '.' + self.extension))
...
dataset = ZipDataset([vid, bgr], transforms=A.PairCompose([
    A.PairApply(T.Resize(args.video_resize[::-1])
                if args.video_resize else nn.Identity()),
    HomographicAlignment() if args.preprocess_alignment else A.PairApply(nn.Identity()),
    A.PairApply(T.ToTensor())
]))
if args.video_target_bgr:
    dataset = ZipDataset([dataset, VideoDataset(
        args.video_target_bgr, transforms=T.ToTensor())])
...
if args.output_format == 'video':
    ...
    if 'com' in args.output_types:
        com_writer = VideoWriter(os.path.join(
            args.output_dir, 'com.mp4'), vid.frame_rate, w, h)
    if 'pha' in args.output_types:
        pha_writer = VideoWriter(os.path.join(
            args.output_dir, 'pha.mp4'), vid.frame_rate, w, h)
        pha_small_writer = VideoWriter(os.path.join(
            args.output_dir, 'pha_sm.mp4'), vid.frame_rate, w, h)
    if 'fgr' in args.output_types:
        fgr_writer = VideoWriter(os.path.join(
            args.output_dir, 'fgr.mp4'), vid.frame_rate, w, h)
    if 'err' in args.output_types:
        err_writer = VideoWriter(os.path.join(
            args.output_dir, 'err.mp4'), vid.frame_rate, w, h)
    if 'ref' in args.output_types:
        ref_writer = VideoWriter(os.path.join(
            args.output_dir, 'ref.mp4'), vid.frame_rate, w, h)
    if 'depth' in args.output_types:
        depth_writer = VideoWriter(os.path.join(
            args.output_dir, 'depth.mp4'), vid.frame_rate, w, h)
    if 'depthdiff' in args.output_types:
        depth_diff_writer = VideoWriter(os.path.join(
            args.output_dir, 'depth_diff.mp4'), vid.frame_rate, w, h)
else:
    if 'com' in args.output_types:
        com_writer = ImageSequenceWriter(
            os.path.join(args.output_dir, 'com'), 'png')
    if 'pha' in args.output_types:
        pha_writer = ImageSequenceWriter(
            os.path.join(args.output_dir, 'pha'), 'jpg')
    if 'fgr' in args.output_types:
        fgr_writer = ImageSequenceWriter(
            os.path.join(args.output_dir, 'fgr'), 'jpg')
    if 'err' in args.output_types:
        err_writer = ImageSequenceWriter(
            os.path.join(args.output_dir, 'err'), 'jpg')
    if 'ref' in args.output_types:
        ref_writer = ImageSequenceWriter(
            os.path.join(args.output_dir, 'ref'), 'jpg')


# Conversion loop
with torch.no_grad():

    bgr_trans = T.Compose([(T.Resize(args.video_resize[::-1])
                            if args.video_resize else nn.Identity()), T.ToTensor()])
    bgr_ref = torch.unsqueeze(bgr_trans(bgr[0]), dim=0).to(
        device, non_blocking=True)
    if args.model_type == 'mattingbase':
        _, _, err, _, depth = model(bgr_ref, bgr_ref)
    elif args.model_type == 'mattingrefine':
        _, _, _, _, err, ref, depth = model(bgr_ref, bgr_ref)
    bgr_depth = depth.detach().clone()

    if args.model_type == 'mattingrefine':
        model.set_bg(bgr_depth)
    ...
    for input_batch in tqdm(DataLoader(dataset, batch_size=1, pin_memory=True)):
        if args.video_target_bgr:
            ...
            src, bgr = input_batch
            tgt_bgr = torch.tensor(
                [120/255, 255/255, 155/255], device=device).view(1, 3, 1, 1)
        ...
        if args.model_type == 'mattingbase':
            ...
            pha, fgr, pha_sm, _, err, ref, depth = model(src, bgr)
        ...
        if 'pha' in args.output_types:
            ...
            pha_small_writer.add_batch(F.interpolate(
                pha_sm, src.shape[2:], mode='bilinear', align_corners=False))
        if 'fgr' in args.output_types:
            fgr_writer.add_batch(fgr)
        if 'err' in args.output_types:
            err_writer.add_batch(F.interpolate(
                err, src.shape[2:], mode='bilinear', align_corners=False))
        if 'ref' in args.output_types:
            ref_writer.add_batch(F.interpolate(
                ref, src.shape[2:], mode='nearest'))
        if 'depth' in args.output_types:
            depth_writer.add_batch(F.interpolate(
                Normalize(depth), src.shape[2:], mode='nearest'))
        if 'depthdiff' in args.output_types:
            diff = Normalize(depth - bgr_depth)
            diff = (diff > 0) * diff
            depth_diff_writer.add_batch(F.interpolate(
                ((diff > diff.max() / 5) * diff), src.shape[2:], mode='nearest'))
